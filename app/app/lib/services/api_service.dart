...
String humanizeApiError(Object error) {
  ...
  if (lower.contains('handshakeexception') ||
    ...
    return 'Échec de la connexion sécurisée (certificat ou TLS). Vérifiez la date et l\'heure du système.';
  }

  if (lower.contains('formatexception') ||
      (lower.contains('json') && lower.contains('parse'))) {
    return 'Réponse du serveur illisible. Le service est peut-être en maintenance.';
  }

  if (RegExp(r'error 5\d\d').hasMatch(lower) ||
      lower.contains('server error') ||
      lower.contains('internal server error')) {
    return 'Le serveur signale une erreur interne. Réessayez plus tard ou contactez le support.';
  }
...
class ApiService {
  ...
  Future<bool> loadSavedCredentials() async {
    try {
      ...
      // Keystore inaccessible (ex: biometrie changée) — reset propre
  ...
  Future<Map<String, dynamic>> extractVideoUrlServer(String url) async {
    return (await _post('extract', {'url': url}, timeout: AppConstants.extractTimeout))
        as Map<String, dynamic>;
  }
  ...
  Future<Map<String, dynamic>?> getProgress(int contentId, {String? episodeId}) async {
    var endpoint = 'progress/get/$contentId';
    if (episodeId != null && episodeId.isNotEmpty) {
      endpoint += '?episode_id=${Uri.encodeComponent(episodeId)}';
    }
